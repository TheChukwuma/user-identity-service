...
-- Assign all permissions to ADMIN role (with conflict handling)
...
-- Assign basic permissions to USER role (with conflict handling)
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r, permissions p
WHERE r.name = 'USER' 
...
-- Assign moderation permissions to MODERATOR role (with conflict handling)
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r, permissions p
